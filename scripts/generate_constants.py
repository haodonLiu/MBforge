...
def generate_rust(data: dict) -> str:
    ...
    lines.append('        return home.join(".cache").join("mbforge").join("models");')
    lines.append("    }")
    lines.append('    PathBuf::from(".cache/mbforge/models")')
    lines.append("}")
    lines.append("")
...
def generate_python(data: dict) -> str:
    ...
    lines.append("            return cfg.model_cache_dir")
    lines.append("    except Exception:")
    lines.append("        pass")
    lines.append("    return str(Path.home() / MODEL_CACHE_DIR.replace(\".\", \"\").replace(\"/\", os.sep).replace(\"~\", str(Path.home())))")
    lines.append("")
    lines.append("")
