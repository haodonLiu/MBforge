...
> 最后更新: 2026-06-04 | 版本: 0.2.2
> 本文档记录项目每个模块的功能、依赖关系、I/O 和实现状态。
> ⚠️ 本次修订基于实际代码审查，补充了断链、配置不同步和安全风险排查。
>
> 规范约束引用：[`docs/specs/`](docs/specs/)（架构约定、代码风格、分子表示三层规范）
...
> 分子存储格式约束见 [`docs/specs/molecular-representation.md`](docs/specs/molecular-representation.md)
| `resource_manager` | 统一资源管理: 11 资源注册 + 路径检查 + GPU 检测 | 无 | ✅ |
| `molecule/` | **分子子目录** (5 模块) | | |
| → `molecule_store` | 分子记录数据库: FTS5 + 属性估算(无 RDKit)。**当前主字段为 `esmiles`，目标迁移为 `smiles`** | constants, molecule_db | ✅ |
...
PdfParseResult ──────────────────────────────────────────────┐
  ...
  ├─ esmiles: Vec<String>  ← 目标: 改为 smiles Vec<String>     │
  └─ parser_used: String                                     │
       │                                                     │
       ▼                                                     │
...
ExtractionResult ← association.rs (图像分子)                  │
  ├─ esmiles: String  ← 目标: 改为 smiles + 可选 esmiles/tags │
  ├─ source: "image" | "text"                                │
