...
use mbforge_chem::chem;
use mbforge_chem::esmiles::{self as esmiles, EsTag};
use mbforge_chem::gesim;
use mbforge_chem::markush;
use mbforge_chem::preprocess;
use mbforge_pipeline::chem::chem_validate;
...
pub fn sar_build_matrix(
    compounds: Vec<mbforge_chem::sar::CompoundInput>,
    core_smiles: Option<String>,
) -> mbforge_chem::sar::RGroupMatrix {
    mbforge_chem::sar::sar_build_matrix(compounds, core_smiles)
}

/// 基于 R-group 矩阵构建活性热力图
#[tauri::command]
pub fn sar_heatmap(
    matrix: mbforge_chem::sar::RGroupMatrix,
    lower_is_better: bool,
) -> Vec<mbforge_chem::sar::ActivityHeatmap> {
    mbforge_chem::sar::sar_heatmap(matrix, lower_is_better)
}
