...
fn output_dir() -> PathBuf {
    let dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../e2e_test/output");
...
/// 提取文本（按 PDF 类型路由）
fn extract_text_routed(
    pdf_path: &str,
    classification: &mbforge::commands::pdf::PdfClassification,
) -> (String, String, usize) {
    ...
    if classification.pdf_type == "Scanned" || classification.pdf_type == "ImageBased" {
        ...
        assert!(
            !api_key.is_empty(),
            "MINERU_API_KEY required for scanned PDF"
        );
    ...
    let result =
        mbforge::commands::pdf::extract_text(pdf_path.to_string()).expect("extract_text failed");
    let dt = t0.elapsed();
    println!(
        "    pdf_inspector 完成: {} chars, {:.2?}",
        result.markdown.len(),
        dt
    );
    (
        result.markdown,
        "pdf_inspector".to_string(),
        result.page_count,
    )
}
...
fn run_pipeline(label: &str, pdf_path: &str) {
    ...
    let t0 = std::time::Instant::now();
    let classification =
        mbforge::commands::pdf::classify_pdf(pdf_path.to_string_lossy().to_string())
            .expect("classify_pdf failed");
    let dt = t0.elapsed();
    println!(
        "  type={}  conf={:.2}  pages={}  needs_ocr={}  ({:.2?})",
        classification.pdf_type,
        classification.confidence,
        classification.page_count,
        classification.pages_needing_ocr.len(),
        dt
    );
    write_json(
        &format!("{}_01_classification.json", label),
        &classification,
    );

    // ── Stage 2: 文本提取（自动路由） ──
    println!("\n[Stage 2] Text Extraction (auto-routed)");
    let (content, parser_used, page_count) =
        extract_text_routed(&pdf_path.to_string_lossy(), &classification);
    ...
    let mol = doc_class
        .pages
        .iter()
        .filter(|p| p.has_molecular_patterns)
        .count();
    println!(
        "  text_density={:.1}  scanned_pages={}/{}  mol_pages={}/{}  ({:.2?})",
        doc_class.text_density,
        scanned,
        doc_class.pages.len(),
        mol,
        doc_class.pages.len(),
    ...
    write_json(
        &format!("{}_04_headings_sections.json", label),
        &serde_json::json!({
    ...
    let dt = t0.elapsed();
    println!(
        "  esmiles={}  activities={}  associated={}  ({:.2?})",
        esmiles.len(),
        activities.len(),
        associated.len(),
        dt
    ...
    write_json(
        &format!("{}_05_molecules.json", label),
        &serde_json::json!({
    ...
    let project_root = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../e2e_test");
    ...
    write_json(
        &format!("{}_07_search_results.json", label),
        &search_results,
    );
    ...
    let doc_ids: Vec<String> = conn
        .prepare("SELECT DISTINCT doc_id FROM sections")
        .unwrap()
        .query_map([], |r| r.get(0))
        .unwrap()
        .filter_map(|r| r.ok())
        .collect();
    println!(
        "  sections:{}  fts5:{}  distinct_docs:{:?}",
        count, fts_count, doc_ids
    );
    ...
    let rows = stmt
        .query_map(rusqlite::params![first_q], |r| {
