...
use mbforge_domain::project::resource_manager::DownloadManagerState;
use mbforge_infra::helpers::LockResultExt;
use mbforge_infra::sidecar_client::get_or_init as get_or_init_sidecar_client;
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // 开发模式路径探测：CARGO_MANIFEST_DIR 指向 crates/mbforge-app/，
            // 项目根目录是其上三级目录（src-tauri/crates/mbforge-app → MBForge）
            ...
            if !no_spawn && server_script.exists() {
                ...
                mbforge_domain::project::resource_manager::write_resolved_paths();
                ...
                if let Err(e) = sidecar::spawn_and_start_readers(&sidecar, &app_handle) {
                    ...
                    let health_client = get_or_init_sidecar_client();
                    match health_client {
                        Ok(client) => {
