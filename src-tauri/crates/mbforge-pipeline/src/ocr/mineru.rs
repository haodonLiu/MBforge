...
//!
//! Real implementation is delegated to [`crate::pdf::mineru::MineruClient`].
...
pub async fn run(pdf_path: &str) -> Result<OcrOutput, String> {
    ...
    let inner = tokio::task::spawn_blocking(move || -> Result<RawMineru, String> {
        use crate::pdf::mineru::{MineruClient, MineruOptions};
        let host =
...
struct RawMineru {
    markdown: String,
    images: Vec<ImageRef>,
    ocr_blocks: Vec<crate::doc_types::OcrBlock>,
}
...
trait MineruOptionsExt {
    fn scanned_for(path: &str) -> crate::pdf::mineru::MineruOptions;
}

impl MineruOptionsExt for crate::pdf::mineru::MineruOptions {
    fn scanned_for(path: &str) -> crate::pdf::mineru::MineruOptions {
        crate::pdf::mineru::scanned_pdf_options(path)
    }
}
...
impl crate::ocr::backend::OcrBackend for MineruBackend {
