...
impl ImageService {
    ...
    pub fn persist_extracted_images(
        ...
        let media_dir = project_root
            ...
            .join(&doc_slug);
        let media_dir =
            match crate::core::helpers::assert_within_root_allow_missing(&root_str, &media_dir) {
                Ok(p) => p,
                Err(e) => {
        ...
        extracted
            ...
            .filter_map(|img| {
                ...
                let dest = match crate::core::helpers::assert_within_root_allow_missing(
                    &root_str, &dest,
                ) {
                    Ok(p) => p,
                    Err(e) => {
            ...
            .filter_map(|img| {
                ...
                let source = match img.rel_path.as_ref() {
                    Some(rel) => {
                        ...
                        match crate::core::helpers::assert_within_root_allow_missing(
                            &root_str, &full,
                        ) {
                            Ok(p) => p,
                            Err(e) => {
