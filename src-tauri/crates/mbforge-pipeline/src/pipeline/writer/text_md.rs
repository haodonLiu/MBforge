...
use crate::doc_types::ImageRef as DocTypeImageRef;
use crate::pipeline::error::{PersistError, PipelineError};
use crate::pipeline::writer::markdown_augment::augment_markdown_with_images;
...
pub fn write_text_markdown(
    ...
    doc_id: &str,
    raw_text: &str,
    images: &[crate::pipeline::models::extracted::ImageRef],
    page_count: usize,
    parser_label: &str,
) -> Result<(PathBuf, Vec<ImageVerification>), PipelineError> {
    let dir = super::output_dir(project_root, doc_id);
    let dir = mbforge_infra::helpers::assert_within_root_allow_missing(
    ...
    let path = mbforge_infra::helpers::assert_within_root_allow_missing(
        project_root.to_string_lossy().as_ref(),
        &path,
    )
    .map_err(|e| {
...
mod tests {
    ...
    ) -> crate::pipeline::models::extracted::ImageRef {
        crate::pipeline::models::extracted::ImageRef {
            filename: filename.to_string(),
            page,
            region: None,
    ...
    fn output_dir_layout() {
        let dir = crate::pipeline::writer::output_dir(Path::new("/tmp/proj"), "doc-123");
