...
use mbforge_infra::helpers::{generate_uuid, now_rfc3339};
...
fn notes_path(root: &Path) -> PathBuf {
    root.join(mbforge_infra::config::constants::PROJECT_META_DIR)
        .join("notes.json")
}
...
mod tests {
    ...
    fn write_test_index(dir: &Path, index: &NotesIndex) {
        fs::create_dir_all(dir.join(mbforge_infra::config::constants::PROJECT_META_DIR)).unwrap();
