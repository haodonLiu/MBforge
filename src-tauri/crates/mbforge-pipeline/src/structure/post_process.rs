            ...
            arr.iter()
                .map(|v| {
                    ...
                    let cleaned_smiles = raw_smiles
                        .as_deref()
                        .map(crate::chem::chem_validate::sanitize_esmiles)
                        .filter(|s| !s.is_empty());
                    CompoundEntry {
...
pub async fn post_process_section(
    ...
    let config = MbforgeProviderConfig::from_app_config().map_err(|e| e.to_string())?;
...
impl SectionResult {
    ...
    pub fn into_data(self) -> Option<crate::doc_types::StructuredData> {
        match self.status {
    ...
    fn test_section_result_status_helpers() {
        ...
        let ok = SectionResult {
            ...
            status: SectionStatus::Ok(PostProcessResult {
                report: "r".into(),
                data: crate::doc_types::StructuredData {
                    metadata: crate::doc_types::DocumentMetadata {
                        title: None,
                        authors: vec![],
