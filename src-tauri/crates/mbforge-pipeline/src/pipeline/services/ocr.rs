...
use crate::pipeline::models::extracted::{ImageRef, OcrBlock};
...
fn adapt_image_ref(old: &crate::doc_types::ImageRef) -> ImageRef {
    ImageRef {
...
fn adapt_ocr_block(old: &crate::doc_types::OcrBlock) -> OcrBlock {
    OcrBlock {
        page: old.page,
...
fn adapt_ocr_output(old: crate::ocr::OcrOutput) -> OcrOutput {
    OcrOutput {
        text: old.text,
...
mod tests {
    ...
    fn adapt_ocr_output_copies_all_fields() {
        let old = crate::ocr::backend::OcrOutput {
            text: "sample text".to_string(),
            page_count: 2,
            images: vec![crate::doc_types::ImageRef {
