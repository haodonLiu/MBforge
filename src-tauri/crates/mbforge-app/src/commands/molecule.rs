...
use mbforge_domain::molecule::molecule_db::{MoleculeRelation, RelationType};
use mbforge_domain::molecule::molecule_engine::{
...
pub async fn mol_get_stats(
    state: tauri::State<'_, MoleculeEngineState>,
) -> Result<mbforge_domain::molecule::molecule_db::RelationStats, String> {
    let guard = state.inner.lock().await;
    let engine = guard
        .as_ref()
        .map(|(_, e)| e)
...
#[tauri::command]
pub async fn chem_validate_smiles(smiles: String) -> mbforge_chem::chem::SmilesValidation {
    mbforge_chem::chem::validate_smiles(&smiles)
}
...
pub async fn chem_tanimoto_similarity(smiles_a: String, smiles_b: String) -> Result<f64, String> {
    mbforge_chem::chem::tanimoto_similarity(&smiles_a, &smiles_b)
}
