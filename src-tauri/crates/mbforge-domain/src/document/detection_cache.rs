...
use mbforge_infra::config::constants::{INDEX_DIR, PROJECTS_DIR};
use mbforge_infra::error::AppResult;
...
impl DetectionCache {
    ...
    fn write_schema_version(&self) -> AppResult<()> {
        ...
        mbforge_infra::helpers::atomic_write(self.schema_version_path(), version.as_bytes())?;
        Ok(())
    }
    ...
    pub fn put(&self, entry: &PageDetection) -> AppResult<()> {
        ...
        mbforge_infra::helpers::atomic_write(&path, json.as_bytes())?;
