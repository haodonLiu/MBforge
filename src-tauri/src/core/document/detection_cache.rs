...
//! `index/pages/<doc_id>/page_<N>.txt` by the caller. This keeps the cache
//! ~50% smaller.
//!
...
pub struct Detection {
    ...
    pub bbox_pdf: [f64; 4],
    /// Canonical SMILES (clean, no semantic tags).
    pub smiles: String,
    /// E-SMILES (with semantic tags, may be empty).
    pub esmiles: String,
...
mod tests {
    ...
    fn sample_entry(doc_id: &str, page: usize, hash: &str) -> PageDetection {
        PageDetection {
            ...
            detections: vec![Detection {
                ...
                esmiles: "<a>C:ALK</a>CCO".to_string(),
                conf_moldet: 0.95,
                conf_molscribe: 0.88,
                vlm_caption: Some("Ethanol molecule diagram".to_string()),
                vlm_esmiles: None,
                crop_relpath: "reports/figures/x/page_001_mol_000.png".to_string(),
            }],
        }
    }
    ...
    fn put_get_round_trip() {
        ...
        assert_eq!(loaded.detections[0].smiles, "CCO");
