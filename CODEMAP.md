...
### 3.3 parsers/ — PDF 解析管线 (16 模块, ~4,700 行)

| 模块 | 行数 | 功能 | 依赖 |
|------|------|------|------|
| `types` | 241 | 管线类型: PdfParseResult/StructuredData/DocStructure/ExtractionPlan 等 | commands::classifier, extractor, core::types |
| `pipeline` | 1039 | **主管线**: Stage 0~7 编排 + 项目级批量索引 | 几乎所有 parsers + core |
...
parsers/pipeline ─→ commands/{classifier, extractor, text_ops}
                   ─→ parsers/{types, headings, sections, post_process, intent, vlm_chem, images, report, uniparser, mineru, llama_parse, liteparse}
...
| # | 功能 | 承诺位置 | 当前状态 | 断链原因 |
|---|------|---------|---------|---------|
| 1 | **semantic_cache** | core/semantic_cache.rs (452 行) | ⚠️ 已实现但未接入 | 未被 executor 或 agent 调用，config 中有 `OptimizationConfig` 但未生效 |
| 2 | **stream_search** | core/stream_search.rs (181 行) | ⚠️ 已实现但未接入 | 未被 knowledge_base 或前端调用 |
...
| 5 | **Python KnowledgeBase** | core/knowledge_base.py | ⚠️ 双份实现 | Rust `knowledge_base.rs` 用 FTS5，Python 版用 ChromaDB，功能重叠 |
