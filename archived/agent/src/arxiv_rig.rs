...
//!
//! rustc's dead_code analysis can't see through `#[rig_tool]` macro
//! expansion, so the generated structs and the original functions both
//! report "never used" even though they ARE registered at runtime.
//! Suppress at the module level and document the call site.

// Tool structs (generated by #[rig_tool]) and the source functions are
// registered by `rig_adapter::assemble_rig_tool_vec!`. rustc can't see
// through `#[rig_tool]` macro expansion, so suppress the false-positive
// "never used" warnings at module level. Wiring is real — see
// rig_adapter::assemble_rig_tool_vec.
