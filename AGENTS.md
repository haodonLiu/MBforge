...
2. **新增 Rust Agent 工具**（rig-core 模式）：
   - 在 `src-tauri/src/core/agent/executor_rig.rs` 中声明 `struct <Name>Tool` + `#[derive(Deserialize, JsonSchema)]` Args
   - 实现 `impl Tool for <Name>Tool`，`call()` 中调用 `core/agent/{fs,kb,document,molecule}.rs` 中的 `native_*` 自由函数
