...
/// Priority: env vars > user cache (~/.cache/mbforge/pdfium/) > vendor/ > auto-download.
fn resolve_pdfium_dirs() -> (PathBuf, PathBuf) {
    ...
    // 2. User cache directory — MBForge dev + packaged install share this path.
    //    Cross-platform: Windows maps $HOME to USERPROFILE, so the path is
    //    `C:\Users\<user>\.cache\mbforge\pdfium\` on Windows and
    //    `~/.cache/mbforge/pdfium/` on Linux/macOS.
    if let Ok(home) = env::var("HOME").or_else(|_| env::var("USERPROFILE")) {
        let cache_lib = PathBuf::from(&home).join(".cache/mbforge/pdfium/lib");
        let cache_include = PathBuf::from(&home).join(".cache/mbforge/pdfium/include");
        if cache_lib.exists() && cache_include.exists() {
            return (cache_lib, cache_include);
        }
    }

    // 3. Vendor directory relative to workspace root (legacy)
