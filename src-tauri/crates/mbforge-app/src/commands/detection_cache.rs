        ...
        match Project::open(&project_path).and_then(|p| p.get_document_source_path(&doc_id)) {
            ...
            None => {
                ...
                let path = project_path
                    .join(mbforge_infra::config::constants::PAPERS_DIR)
...
pub fn clear_detection_cache(project_root: String) -> Result<(), String> {
    ...
    DetectionCache::new(&root)
        .clear_all()
        .map_err(|e: mbforge_infra::error::AppError| e.to_string())
}
...
/// - `sidecar_url`: sidecar base URL（默认 `mbforge_infra::config::constants::sidecar_url()`）
...
pub async fn vlm_chem_coref(
    ...
    image_w: u32,
    image_h: u32,
) -> Result<mbforge_pipeline::chem::vlm_chem::CorefOutput, String> {
    use mbforge_pipeline::chem::vlm_chem;
    ...
    let url = sidecar_url.unwrap_or_else(mbforge_infra::config::constants::sidecar_url);
...
) -> Result<Option<mbforge_pipeline::chem::label_assoc::LabelMatch>, String> {
    use mbforge_pipeline::chem::label_assoc;
