...
use mbforge_infra::helpers::now_secs_f64;
...
impl SemanticCache {
    pub fn new(project_root: &Path, config: SemanticCacheConfig) -> Self {
        let db_path = project_root
            .join(mbforge_infra::config::constants::INDEX_DIR)
        ...
        F: FnOnce(&Connection) -> AppResult<T>,
    {
        if let Some(shared) = &self.shared_conn {
            ...
            let guard = shared.lock().map_err(|e| mbforge_infra::error::AppError {
                code: mbforge_infra::error::ErrorCode::Unknown,
