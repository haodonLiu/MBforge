...
- **Lines:** 26
- **Logic:** Re-exports `MoleculeExtractor` from parent module. Local PDF processing track (parallel to parser_io/ UniParser API track).

---

## 56. `src/mbforge/parser_io/__init__.py`
- **Path:** `C:\Users\10954\Desktop\MBForge\src\mbforge\parser_io\__init__.py`
- **Lines:** 19
- **Logic:** Exports `ParserConfig`, `load_config`, `validate_config`, `ParseResult`, `MoleculeData`, `SARTask`, `ParserClient`.

---

## 57. `src/mbforge/parser_io/models.py`
- **Path:** `C:\Users\10954\Desktop\MBForge\src\mbforge\parser_io\models.py`
- **Lines:** 57
- **Imports:** Standard library: `dataclasses`, `typing`
- **Class `ParseResult` (line 12, dataclass):** status, token, raw_data.
- **Class `MoleculeData` (line 27, dataclass):** smiles, name, activity, source.
- **Class `SARTask` (line 44, dataclass):** molecules (List[MoleculeData]), metadata.
- **Logic:** Data models for UniParser integration. ParseResult holds API response, MoleculeData is the extracted molecule, SARTask bundles molecules for openSAR analysis.

---

## 58. `src/mbforge/parser_io/config.py`
- **Path:** `C:\Users\10954\Desktop\MBForge\src\mbforge\parser_io\config.py`
- **Lines:** 78
- **Imports:** Standard library: `os`, `dataclasses`, `pathlib.Path`, `typing`. Deferred: `dotenv`
- **Class `ParserConfig` (line 20, dataclass):** host, api_key.
- **Functions:**
  - `find_env_file() -> Optional[Path]` (line 27): Searches CWD and parents for `.env`.
  - `load_config() -> ParserConfig` (line 40): Loads `UNIPARSER_HOST` and `UNIPARSER_API_KEY` from env.
  - `validate_config(config) -> bool` (line 69): Checks host URL format and api_key presence.
- **Logic:** Configuration management for UniParser integration. Loads host/API key from `.env` file or environment variables.

---

## 59. `src/mbforge/parser_io/client.py`
- **Path:** `C:\Users\10954\Desktop\MBForge\src\mbforge\parser_io\client.py`
- **Lines:** 212
- **Imports:** Standard library: `time`, `pathlib.Path`, `typing`. Deferred: `uniparser_tools.api.clients.UniParserClient`, `uniparser_tools.common.constant.ParseMode`, `uniparser_tools.common.constant.ParseModeTextual`. Internal: `.config.ParserConfig`, `.models.ParseResult`
- **Class `ParserClient` (line 27):**
  - Methods:
    - `__init__(config)` (line 39): Creates UniParserClient base instance.
    - `parse_pdf(pdf_path, sync, textual, table, equation, chart, figure, expression, molecule, **kwargs) -> ParseResult` (line 51): Triggers PDF parsing with per-element mode configuration.
    - `get_result(token, content, objects, pages_dict, pages_tree, molecule_source) -> Dict` (line 105)
    - `get_formatted(token, content, textual, table, equation) -> Dict` (line 137): Markdown-formatted output.
    - `parse_and_wait(pdf_path, timeout, poll_interval, **kwargs) -> ParseResult` (line 166): Async parse with polling.
    - `health() -> Dict` (line 205)
    - `version() -> Dict` (line 209)
- **Logic:** Wrapper around UniParser-Tools SDK. Provides simplified interface for PDF parsing with configurable element modes (OCR quality, dump, fast), async polling, and health checks.
