...
pub use mbforge_infra::types::{Heading, SectionChunk, TreeNode};
...
/// 从文本中提取所有 heading。
///
/// 输入应来自 pdf-inspector 的 markdown 输出，其中标题已被规范化为
/// Markdown H1-H6。本函数不再猜测专利/论文的特殊格式。
...
pub fn build_tree(sections: &[SectionChunk]) -> Vec<TreeNode> {
    ...
    for (i, section) in sections.iter().enumerate() {
        ...
        while let Some(top) = stack.last() {
            let top_level = tree_node_path_level(top);
            if top_level >= level {
...
fn tree_node_path_level(node: &TreeNode) -> usize {
    // 通过节点在树中的深度估算 level
    // 简化：用 title 中的数字前缀判断
    if let Some(num) = node.title.split('.').next() {
