...
async fn pdf_hash_cached(pdf_abs: &std::path::Path) -> Option<String> {
    ...
    let hash = mbforge_infra::helpers::sha256_file(pdf_abs).ok()?;
...
fn detection_match_key(det: &mbforge_domain::document::detection_cache::Detection) -> String {
...
fn composite_conf(det: &mbforge_domain::document::detection_cache::Detection) -> f64 {
        ...
        .and_then(|p| p.get_document_source_path(doc_id))
    {
        Some(path) => {
            let slug = path
                ...
                .unwrap_or(doc_id)
                .to_string();
            let legacy = path.starts_with(root.join(mbforge_infra::config::constants::PAPERS_DIR));
            (slug, legacy)
        }
...
pub async fn ensure_coref_for_image(
    ...
    use mbforge_domain::document::knowledge_base::get_or_init_kb;
    use mbforge_pipeline::pipeline::services::coref_persist::CorefPersistService;
...
pub fn update_coref_pair(
    ...
    use mbforge_domain::document::knowledge_base::CorefPrediction;
