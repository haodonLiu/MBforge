//! 9 literature tools for the rig-core agent stack.
//!
//! Wraps the HTTP client in `crate::core::agent::arxiv` (Agentic Data API for
//! arXiv / bioRxiv / medRxiv / PubMed Central) as rig-core tools via the
//! `#[rig_tool]` macro. The macro generates `<Name>` unit structs and their
//! `Tool` impl from the function bodies below.
//!
//! Together with the 16 tools in `executor_rig.rs`, these form the 25-tool
//! set assembled by `rig_adapter::assemble_rig_tool_vec`.
//!
//! Tools provided: `arxiv_metadata`, `arxiv_brief`, `arxiv_preview`,
//! `arxiv_raw`, `arxiv_section`, `arxiv_search`, `arxiv_trending`,
//! `pmc_metadata`, `pmc_json`.
