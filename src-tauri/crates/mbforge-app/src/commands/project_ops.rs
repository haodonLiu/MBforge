...
use mbforge_infra::helpers::{clean_path, LockResultExt};
use mbforge_pipeline::pipeline::writer::output_status::{output_status, IncompleteReason};
...
pub fn open_project(
    ...
    if let Some(project) = mbforge_domain::project::Project::open(&path) {
    ...
    // 目录存在但不是项目 → 创建
    match mbforge_domain::project::Project::create(&path) {
        Some(project) => {
...
pub fn scan_project_files(root: String) -> Result<serde_json::Value, String> {
    ...
    let path = PathBuf::from(&root);
    let mut project = mbforge_domain::project::Project::open(&path).ok_or_else(|| {
...
fn warnings_json(warnings: &[mbforge_domain::project::ScanWarning]) -> Vec<serde_json::Value> {
    warnings
        .iter()
        .map(|w| {
            serde_json::json!({
...
fn docs_json(docs: &[mbforge_domain::project::project::DocumentEntry]) -> Vec<serde_json::Value> {
    docs.iter().map(doc_json).collect()
}

fn doc_json(doc: &mbforge_domain::project::project::DocumentEntry) -> serde_json::Value {
    serde_json::json!({
        "doc_id": doc.doc_id,
        "path": doc.path,
...
fn project_json(project: &mbforge_domain::project::Project) -> serde_json::Value {
...
pub fn list_project_documents(root: String) -> Result<serde_json::Value, String> {
    let root = clean_path(&root);
    let path = PathBuf::from(&root);
    let project = mbforge_domain::project::Project::open(&path).ok_or_else(|| {
        AppError::new(ErrorCode::ProjectOpen, format!("项目不存在: {root}")).to_string()
    })?;
...
pub async fn enqueue_unresolved_documents(root: String) -> Result<serde_json::Value, String> {
    ...
    let queue = mbforge_domain::ingest_queue::IngestQueue::new(&path)
...
fn start_or_restart_ingest_worker(app: &tauri::AppHandle, project_root: &std::path::Path) {
    use mbforge_pipeline::ingest_worker::{IngestWorker, IngestWorkerState};
