...
| R-13 | Pipeline | Phase 1 Activity Row-Alignment: 从 page-proximity 升级为按 (table_idx, row_idx) 行级匹配 + name → smiles → page-proximity 三级降级。 | `src/mbforge/pipeline/extract_activities.py`, `persist_molecules.py` | **RESOLVED 2026-07-10** (name → smiles → page-proximity fallback; _link_activity_to_molecule + 5 tests) |
