fn main() {
    ...
    let vendor_lib = std::path::PathBuf::from(&manifest_dir).join("vendor/pdfium/release/lib");
    let vendor_include =
        std::path::PathBuf::from(&manifest_dir).join("vendor/pdfium/release/include");
    ...
    if vendor_lib.exists() && vendor_include.exists() {
        ...
        if std::env::var("PDFIUM_INCLUDE_PATH").is_err() {
            println!(
                "cargo:rustc-env=PDFIUM_INCLUDE_PATH={}",
                vendor_include.display()
            );
        }
    }
